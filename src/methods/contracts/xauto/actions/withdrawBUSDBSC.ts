...
import WithdrawSavingsBUSDXAuto from "../methods/withdrawBUSDBSCXAuto";
import { rehydrateVault } from "../../../hydrate";


function WithdrawBUSDBSCXAuto(amount: any, addressOwner: string, chainId: any) {
    return async (dispatch: Function) => {

        try {
            notify('success', 'USDT Matic Deposit Initiated');
            // dispatch(loader(id));
            const response = await WithdrawSavingsBUSDXAuto(amount, addressOwner, chainId);
...
export default WithdrawBUSDBSCXAuto;