...
import WithdrawSavingsUSDCXBSCAuto from "../methods/withdrawUSDCBSCXAuto";



function WithdrawUSDCBSCXAuto(amount: any,addressOwner:string,chainId:any) {
    return async (dispatch: Function) => {
        
        try {
            notify('success', 'USDT Matic Deposit Initiated');
           // dispatch(loader(id));
            const response = await WithdrawSavingsUSDCXBSCAuto(amount,addressOwner,chainId);
...
export default WithdrawUSDCBSCXAuto;