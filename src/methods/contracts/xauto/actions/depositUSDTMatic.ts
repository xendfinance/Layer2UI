...
import DepositSavingsUSDTMatic from "../methods/depositUSDTMaticXAuto";
import { rehydrateVault } from "../../../hydrate";


function DepositUSDTMatic(amount: any, addressOwner: string, chainId: any) {
    return async (dispatch: Function) => {

        try {
            notify('success', 'USDT Matic Deposit Initiated');
            // dispatch(loader(id));
            const response = await DepositSavingsUSDTMatic(amount, addressOwner, chainId);
...
export default DepositUSDTMatic;