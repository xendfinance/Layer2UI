...
import WithdrawSavingsUSDTMatic from "../methods/withdrawUSDTMaticXAuto";


function WithdrawUSDTMatic(amount: any,addressOwner:string,chainId:any) {
    return async (dispatch: Function) => {
        
        try {
            notify('success', 'USDT Matic Deposit Initiated');
           // dispatch(loader(id));
            const response = await WithdrawSavingsUSDTMatic(amount,addressOwner,chainId);
...
export default WithdrawUSDTMatic;