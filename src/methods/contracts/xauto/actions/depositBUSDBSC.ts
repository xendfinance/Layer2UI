...
import DepositSavingsBUSDXAuto from "../methods/depositBUSDBSCXAuto";
import { rehydrateVault } from "../../../hydrate";


function DepositBUSDBSCXAuto(amount: any, addressOwner: string, chainId: any) {
    return async (dispatch: Function) => {

        try {
            notify('success', 'USDT Matic Deposit Initiated');
            // dispatch(loader(id));
            const response = await DepositSavingsBUSDXAuto(amount, addressOwner, chainId);
...
export default DepositBUSDBSCXAuto;