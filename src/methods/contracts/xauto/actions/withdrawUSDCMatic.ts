...
import WithdrawSavingsUSDCMatic from "../methods/withdrawUSDCMaticXAuto";


function WithdrawUSDCMatic(amount: any,addressOwner:string,chainId:any) {
    return async (dispatch: Function) => {
        
        try {
            notify('success', 'USDT Matic Deposit Initiated');
           // dispatch(loader(id));
            const response = await WithdrawSavingsUSDCMatic(amount,addressOwner,chainId);
...
export default WithdrawUSDCMatic;