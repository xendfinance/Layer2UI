...
import WithdrawSavingsUSDC from "../methods/withdrawUSDC";


function WithdrawUSDCBsc(amount: any,addressOwner:string,chainId:any) {
    return async (dispatch: Function) => {
        
        try {
            notify('success', 'USDT BSC Withdraw Initiated');
           // dispatch(loader(id));
            const response = await WithdrawSavingsUSDC(amount,addressOwner,chainId);
...
export default WithdrawUSDCBsc;